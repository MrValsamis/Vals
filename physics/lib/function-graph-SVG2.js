...
function derivative(f, x) {
  ...
  if (showMouseCoords) {
    ...
    let circle = document.getElementById(id + "-dot");
    if (!circle) {
        circle = document.createElementNS("http://www.w3.org/2000/svg", "circle");
        circle.setAttribute("id", id + "-dot");
        circle.setAttribute("r", 3);  // radius of the circle
        circle.style.fill = "#000";  // fill color of the circle
        target.appendChild(circle);
    }

    let x = mouse.x;
    let y = y0 - yScale * func((mouse.x - x0) / xScale);
    circle.setAttribute("cx", x);
    circle.setAttribute("cy", y);

    let slope = derivative(func, (mouse.x - x0) / xScale);
    let x1 = mouse.x - 50;  // adjust as needed
    let y1 = y0 - yScale * (func((mouse.x - x0) / xScale) + slope * ((x1 - mouse.x) / xScale));
    let x2 = mouse.x + 50;  // adjust as needed
    let y2 = y0 - yScale * (func((mouse.x - x0) / xScale) + slope * ((x2 - mouse.x) / xScale));

    // Update or create the SVG line element representing the segment
    let segment = document.getElementById(id + "-segment");
    if (!segment) {
        segment = document.createElementNS("http://www.w3.org/2000/svg", "line");
        segment.setAttribute("id", id + "-segment");
        segment.style.stroke = "red";  // or whatever style you prefer
    ...
    // Update text to show slope instead of coordinates
     let textElement = document.getElementById(id + "-coords");
    textElement.setAttribute("x", mouse.x + 5);  // Adjust this value to move the text left or right
    textElement.setAttribute("y", y0 - yScale * func((mouse.x - x0) / xScale) + 10);  // Adjust this value to move the text up or down
    textElement.textContent = `Slope: ${slope.toFixed(2)} m/s`;
});
    }
  }
